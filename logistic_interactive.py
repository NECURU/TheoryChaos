...
txtfont = QtGui.QFont("Avenir Next") # требуется макрос
...
def logistic_map(pop, rate):
    ...
    ---------
    pop: float
       текущая популяция в момент времени t
    rate: float
       значения параметра скорости роста

    Returns
    -------
    float
        скалярный результат логистической карты в момент времени t + 1
    """
...
def get_function_points(model, r, n, start, end):
    ...
    ---------
    model: function
        определение повторяющейся карты для моделирования
    r: float
        значение параметра скорости роста для передачи на карту
    n: int
      количество итераций для запуска
    start: float
       нижняя граница диапазона функций
    end: float
        верхняя граница диапазона функций
